...
sub process_log($) {
	...
	if ($current_mod_t == $PREV_MOD_T) {
		...
		debug("No update since ".mod_t_as_string($current_mod_t));
		return 1;
	};
	...
	if (!open(F,"<$LOGFILE")) { 
		debug("Can't read first line of logfile"); 
		return 1; 
	};
	$new_head=<F>;
	close(F);
...
WWW
		...
		foreach $line (@alerts) {
			...
			print "<tr class='rlvl'".$lvl."><td class='aclvl'".$lvl." bgcolor='$color' width='60px'>&nbsp;</td><td class='tclvl'".$lvl." width='100%'>$alrt</td></tr>\n";
...
ZZZ
	...
	foreach my $conf (@confs) {
		...
		if (process_log("$conf")!=0) {
			debug("We had an error procssing the logfile defined in $conf");
		} else {
			if (scalar @alerts>0) {
				display_alerts($LOGFILE);
			} else {
				debug("New log lines but no alerts");
			};
		};
		debug("-" x 80);
	};
